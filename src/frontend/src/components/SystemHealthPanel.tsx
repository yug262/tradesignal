...
import { bigIntToDate, formatTimestamp } from "@/types/trading";
...
export function SystemHealthPanel({
  ...
  const lastPollTs = processingState?.last_poll_timestamp ?? 0n;
  const lastPollStr = lastPollTs > 0n ? formatTimestamp(lastPollTs) : "—";

  const lastPollDate =
    lastPollTs > 0n
      ? bigIntToDate(lastPollTs).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        })
      : "";

  const queue = processingState ? Number(processingState.articles_in_queue) : 0;
  const totalProcessed = processingState
    ? Number(processingState.total_articles_processed)
    : 0;
      ...
      <div className="bg-card border border-border rounded p-3 space-y-2.5">
        ...
        <div className="space-y-0">
          <InfoRow
            label="Last Poll"
            value={
              lastPollTs > 0n ? `${lastPollDate} ${lastPollStr} UTC` : "Never"
            }
          />
          <InfoRow
            label="Queue"
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          {isPolling && config && (
            <span className="font-mono text-[9px] text-muted-foreground ml-auto opacity-60">
              every {Number(config.polling_interval_mins)}m
            </span>
          )}
        </div>
      </div>
