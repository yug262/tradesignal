...
import { bigIntToDate } from "@/types/trading";
import {
  AlertTriangle,
  BarChart3,
  Database,
          ...
          <TabsContent value="endpoint" className="mt-3">
            <div className="bg-card border border-border rounded p-5 space-y-5">
              ...
              <SliderField
                ...
                unit=" min"
                onChange={(v) =>
                  update("polling_interval_mins", BigInt(Math.round(v)))
                }
              ...
              {procState ? (
                <div className="bg-background border border-border rounded p-3 space-y-0">
                  ...
                  <StatusRow
                    label="Last Poll"
                    value={
                      procState.last_poll_timestamp > 0n
                        ? `${bigIntToDate(procState.last_poll_timestamp)
                            .toISOString()
                            .replace("T", " ")
                            .slice(0, 19)} UTC`
                        : "Never"
                  ...
                  <StatusRow
                    ...
                    value={Number(
                      procState.total_articles_processed,
                    ).toLocaleString()}
                  />
                  <StatusRow
                    label="Articles In Queue"
                    value={Number(procState.articles_in_queue).toLocaleString()}
                  />
                  <StatusRow
                    label="Current Mode"
