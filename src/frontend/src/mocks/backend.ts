import type { backendInterface, NewsArticleRef, SystemConfig, DashboardSummary, ProcessingState } from "../backend";
...
const mockArticles: NewsArticleRef[] = [
  {
    ...
    analyzed_at: BigInt(Date.now() * 1_000_000),
    ...
    published_at: BigInt((Date.now() - 3600000) * 1_000_000),
    news_relevance: "HIGH",
  ...
  },
  {
    ...
    published_at: BigInt((Date.now() - 7200000) * 1_000_000),
  ...
  },
  {
    ...
    published_at: BigInt((Date.now() - 1800000) * 1_000_000),
    news_relevance: "HIGH",
    processing_status: "PROCESSED",
  ...
  },
  {
    ...
    published_at: BigInt((Date.now() - 5400000) * 1_000_000),
  ...
  },
  {
    ...
    published_at: BigInt((Date.now() - 900000) * 1_000_000),
    news_relevance: "HIGH",
    processing_status: "PROCESSED",
...
const mockConfig: SystemConfig = {
  ...
  min_rr: 2.0,
  max_open_positions: BigInt(5),
  capital: 50000,
  max_daily_loss_pct: 3.0,
  polling_interval_mins: BigInt(15)
};

const mockDashboardSummary: DashboardSummary = {
  endpoint_status: "CONNECTED",
  no_trade_count: BigInt(12),
  system_mode: "EVENT",
  total_articles_consumed: BigInt(1247),
  pending_candidates: BigInt(3),
  last_refresh: BigInt(Date.now() * 1_000_000),
  articles_processed_today: BigInt(47),
  active_opportunities: BigInt(4)
};

const mockProcessingState: ProcessingState = {
  is_polling_active: true,
  last_processed_article_id: "art-005",
  articles_in_queue: BigInt(3),
  current_mode: "EVENT",
  total_articles_processed: BigInt(1247),
  last_poll_timestamp: BigInt((Date.now() - 60000) * 1_000_000)
};

export const mockBackend: backendInterface = {
  fetch_news: async () => "OK: 5 articles fetched",
  filter_news: async (symbol, category, min_impact) => {
    return mockArticles.filter(a => {
      if (symbol && !a.affected_symbols.includes(symbol)) return false;
      if (category && a.news_category !== category) return false;
      if (min_impact !== null && a.impact_score < min_impact) return false;
      return true;
    });
  },
  get_config: async () => mockConfig,
  get_dashboard_summary: async () => mockDashboardSummary,
  get_news: async (page, page_size) => {
    const start = Number(page) * Number(page_size);
    return mockArticles.slice(start, start + Number(page_size));
  },
  get_news_by_id: async (id) => mockArticles.find(a => a.id === id) ?? null,
  get_processing_state: async () => mockProcessingState,
  get_total_news_count: async () => BigInt(mockArticles.length),
  reset_config: async () => mockConfig,
  update_config: async (cfg) => true
};
