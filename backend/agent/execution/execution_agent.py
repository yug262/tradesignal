    ...
    try:
        ...
        for sig in pending_signals:
            ...
            agent3_input = {
                ...
                "live_news_context": agent2_view.get("live_news_context", {}) # Pass live news context if available
            }
...
# =========================================================================
# Trigger Proper Pipeline from Live News (Agent 2.5 -> 3)
# =========================================================================

def trigger_pipeline_from_live_news(
    symbol: str,
    live_news_output: dict,
    db: Session = None,
) -> dict:
    """
    Trigger the full trading pipeline starting from technical analysis (Agent 2.5)
    based on a successful live news analysis result.

    Creates a synthetic confirmed signal and hands it off to Agent 2.5.
    Agent 2.5 will then automatically trigger Agent 3 if technicals are GO.
    """
    own_session = False
    if db is None:
        db = SessionLocal()
    ...
    print(f"\n  [LIVE NEWS -> PIPELINE] Creating signal for {symbol}...")

    try:
        # -- Step 1: Translate live news output -> agent2_view format --
        ...
        # -- Step 2: Create a synthetic confirmed signal --
        sig_id = f"live-news-{symbol}-{started_at}"
        
        synthetic_signal = db_models.DBTradeSignal(
            id=sig_id,
            symbol=symbol,
            signal_type="BUY" if direction == "BULLISH" else "SELL" if direction == "BEARISH" else "WATCH",
            trade_mode="INTRADAY",
        ...
        # -- Step 3: Call Agent 2.5 (Technical Analysis) --
        # This will automatically trigger Agent 3 if technicals look good.
        print(f"  [LIVE NEWS -> PIPELINE] Triggering Agent 2.5 for {symbol}...")
        from agent.technical_analysis.technical_analysis_agent import run_technical_analysis
        ta_result = run_technical_analysis(db, signal_ids=[sig_id])
        ...
        print(f"  [PIPELINE ERROR] {symbol}: {e}")
        traceback.print_exc()
